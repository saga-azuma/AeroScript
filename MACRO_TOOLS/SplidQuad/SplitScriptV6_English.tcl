...
proc ::SplitGUI {imagedir} {
	...
	set w ".splitGUI";
	toplevel $w
	wm title $w "split v6 E";
	wm geometry $w "190x450";
	wm resizable $w 0 0;
	::hwt::KeepOnTop $w;

	### Label ###
	label $w\.selection		-text "Select the split type";
	label $w\.selmethod		-text "Click the closest edge";
...
#	image create photo combineimg -file "$imagedir\/Combine.png";
	...
	checkbutton $w.1dsplit		-text "Split connected 1D" \
	...
	# Comnbine button #
	button $w.combine		-text "Combine panel" \
					-font {{MS Sans Serif} 11 normal roman} \
					-width 20 \
					-height 2 \
					-background green \
